...
export default function PricingServiceCard({ title, description, icon: Icon, link, delay = 0 }) {
  ...
  return (
    ...
    >
      <Link to={lp(link)} className="relative flex h-full flex-col overflow-hidden rounded-2xl bg-card p-8">
        ...
        <div className="relative mt-auto flex items-center font-semibold text-brandkraf-teal">
          {T('View Pricing', 'Lihat Harga')} <ArrowRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
        </div>
      </Link>
    </motion.div>
  );
